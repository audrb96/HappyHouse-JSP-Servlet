...
<body>
		...
		$(() => {
			 ...
			 let doName = "";
			  ...
			  $(document).on("click", "#dong-list li > a", (e) => {
				    ...
				    location.href = "apt?action=search&dong=" + code;
	...
	<div id="map-main" style="display: flex;" >
		<div class="map-left" style="margin-left: 30px; margin-top: 10px;">
			...
			<c:forEach items="${aptlist }" var="apt">
				<div>
					<div class ="apt-name" style="font-size:20px;">${apt.name}</div>
					<div style="font-size:10px;">거래금액 : ${apt.price}만원</div> <div style="font-size:10px;">면적 : ${apt.area}</div>
					<div style="font-size:8px;color:lightgray;">${apt.date}</div>
				</div><hr>
			</c:forEach>
			</div>
